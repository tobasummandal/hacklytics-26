...
const WorldGraph = lazy(() => import('./WorldGraph'))
...
function HighlightTextarea({ value, onChange, highlights, placeholder, containerStyle }: {
  ...
  const highlightedHTML = useMemo(() => {
    ...
    for (const h of highlights) {
      ...
      let idx = 0
      while (true) {
        const found = valueLower.indexOf(hLower, idx)
        if (found === -1) break
...
export default function Dashboard() {
  ...
  const ingestStreamCloseRef = useRef<(() => void) | null>(null)

  const closeIngestStream = useCallback(() => {
    const close = ingestStreamCloseRef.current
    if (close) close()
    ingestStreamCloseRef.current = null
  }, [])
  ...
  const refreshGraph = useCallback(async () => {
    try {
      setGraphData(await api.getGraph({ limit_nodes: 700, limit_edges: 3000 }))
    } catch { }
  }, [])
  ...
  const runCheck = useCallback(async (text: string) => {
    ...
    if (lastCheckedTextRef.current === normalized) return
    if (checkAbortRef.current) checkAbortRef.current.abort()
    const aborter = new AbortController()
    checkAbortRef.current = aborter
    setChecking(true)
    setFlags([])
    setCheckError(null)
    try {
      const result = await api.checkPassage(normalized, CHAPTER, aborter.signal)
      ...
      if (checkAbortRef.current === aborter) {
  ...
  const handleIngest = async () => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        const close = api.ingestEventStream(started.job_id, {
          ...
          onError: () => {
            setIngestError('Ingest stream disconnected. Check backend connectivity.')
            setIngesting(false)
            setIngestProgress(null)
            closeIngestStream()
            resolve()
        ...
        ingestStreamCloseRef.current = close
      })
    } catch (e) {
  ...
  const handleNewTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    setNewText(val)
    setFlags([])
    if (lastCheckedTextRef.current && !val.trim()) {
  ...
  useEffect(() => {
    if (!newText.trim() || newText.trim().split(/\s+/).length < 5) return
